...
if matches[1] == 'delwlc' and not matches[2] and is_owner(msg) then 
   ...
   redis:hdel(hash,'welcome')
        return 'متن خوش آمدگویی پاکسازی شد'
end
...
redis:hset(hash,'welcome',matches[2])
        return 'متن خوش آمدگویی تنظیم شد: ✋\n'..matches[2]
end
...
group_welcome = string.gsub(group_welcome, '{نام}', ""..(msg.action.user.first_name or '').."")
 group_welcome = string.gsub(group_welcome, '{نام خانوادگی}', ""..(msg.action.user.last_name or '').."")
